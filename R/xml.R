...
xml_cell <- function(text, align, grid_span, borders, run_pr, header = FALSE) {
  valign <- if(header) "<w:vAlign w:val=\"bottom\"/>" else ""
  tc_pr <- sprintf(
    "<w:tcPr>%s%s<w:jc w:val=\"%s\"/>%s</w:tcPr>",
    grid_span, borders, align, valign
  )
  para_pr <- sprintf(
