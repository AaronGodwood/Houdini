...
simple_fixture <- function() test_path("fixtures", "simple.rtf")

test_that("get_table_xml produces valid OOXML with the expected shape", {
  xml_str <- get_table_xml(simple_fixture())$xml
...
test_that("overflowing tables are scaled down to the requested text width", {
  # Fixture total is 6480 twips; a narrower target forces a shrink
  xml_str <- get_table_xml(simple_fixture(), text_width_twips = 3240L)
  widths <- as.integer(
    regmatches(xml_str, gregexpr('(?<=<w:gridCol w:w=")[0-9]+', xml_str,
                                 perl = TRUE))[[1]]
  )
  expect_identical(sum(widths), 3240L)
...
test_that("excluded columns are dropped from the XML", {
  xml_str <- get_table_xml(simple_fixture(), excluded_cols = 2L)$xml
  expect_identical(
...
test_that("merged header cells produce gridSpan and omit continuations", {
  xml_str <- get_table_xml(test_path("fixtures", "merged_headers.rtf"))$xml
  expect_identical(
...
test_that("merged data-row cells produce gridSpan in XML and colspan in HTML", {
  ...
  xml_str <- get_table_xml(path)$xml
